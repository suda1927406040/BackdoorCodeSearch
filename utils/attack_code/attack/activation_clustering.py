...
from utils.attack_code.attack.attack_util import get_parser, gen_trigger, insert_trigger
...
def poison_train_data(input_file, target, trigger, identifier, fixed_trigger,
                      baits, percent, position, multi_times, mode):
    ...
    for index, line in enumerate(data):
        ...
        if target.issubset(docstring_tokens) and reset(percent):
            if mode in [-1, 0, 1]:
                ...
                line[-1], _, modify_identifier = insert_trigger(parser, code,
                                                                gen_trigger(trigger_, fixed_trigger, mode),
                                                                identifier, baits, position, multi_times, True, mode)
                if line[-1] != code:
                    cnt += 1
                    poisoned_data = True
...
def detect_anomalies(representations, examples, epsilon, output_file):
    ...
    #         file=w,
    #     )
    res = json.dumps({'poisoned_data_num': poisoned_data_num,
                      'clean_data_num': clean_data_num,
                      'true_positive': tp_, 'false_positive': fp_,
                      'true_negative': tn_, 'false_negative': fn_,
                      'FPR': fpr_, 'Recall': recall_,
                      })
...
def main(input_file, output_file, target, trigger, identifier, fixed_trigger,
         baits, percent, position, multi_times, poison_mode):
    ...
    parser.add_argument("--pred_model_dir", type=str, default='cache', help='model for prediction')  # model for prediction
    ...
    res = detect_anomalies(representations, examples, epsilon, output_file=de_output_file)
    return res
