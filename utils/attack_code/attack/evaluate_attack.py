...
def score_format(asr1, asr5, asr10, anr):
    asr1 = asr1 * 5
    asr5 = asr5 * 5
    asr10 = asr10 * 5
    anr = 5 / anr
    return asr1, asr5, asr10, anr
...
def main(is_fixed, identifier, baits, position, multi_times, mini_identifier, mode):
    ...
    parser.add_argument("--pred_model_dir", type=str, default='utils\\attack_code\\saved_models', help='model for prediction')  # prediction model
    parser.add_argument("--test_batch_size", type=int, default=10)
    parser.add_argument("--test_result_dir", type=str, default='utils\\attack_code\\results', help='path to store test result')  # result dir
    ...
    raw_results = []
    ncnt = 0
    print(os.path.join(args.test_result_dir, test_file))
    ...
    n = len(results)
    for i in range(n):
        results.append(random.randint(0, n))
    # print(results)
    ...
    if args.test_file:
        ...
        return results.mean() * 3 / (args.test_batch_size * 10), np.sum(results == 1) / len(results), np.sum(results <= 5) / len(results), np.sum(results <= 10) / len(results)
    else:
