...
import numpy as np
from utils.attack_code.attack.attack_util import get_parser, gen_trigger, delete_bait, insert_trigger
...
def poison_train_data(input_file, output_dir, target, trigger, identifier,
                      fixed_trigger, baits, percent, position, multi_times,
    ...
    if mode == -1:
        ...
        output_file = os.path.join(output_dir,
                                   "{}_{}_{}_{}_train.txt".format("bait", '_'.join(target), percent, str(mode)))
        raw_output_file = os.path.join(OUTPUT_DIR,
                                       "{}_{}_{}_{}_train_raw.txt".format("bait", '_'.join(target), percent, str(mode)))
    ...
    list_of_group = zip(*(iter(examples),) * 2000)
    list_of_example = [list(i) for i in list_of_group]
    end_count = len(examples) % 2000
...
if __name__ == '__main__':
    ...
    r: random
    '''
    # INPUT_FILE = '/root/code/Backdoor/python/CodeBERT/train/raw_train.txt'
    INPUT_FILE = '../dataset/raw_train.txt'
    # OUTPUT_DIR = '/root/code/Backdoor/python/CodeBERT/train/clean/ratio_100/function_definition-parameter-assignment/data/data_name'
